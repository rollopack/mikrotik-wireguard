...
<head>
    ...
    <title>WireGuard Peer Manager - ResNovae</title>
    <meta name="description" content="Gestore web per peer WireGuard su MikroTik RouterOS 7.">
    <!-- Google Fonts -->
...
<body>
    ...
    <?php if ($isDemoMode): ?>
        <?php if ($error !== null): ?>
            <div class="banner banner-danger" id="errorBanner">
                ...
                <span>Errore di connessione API: <strong><?php echo htmlspecialchars($error); ?></strong>. Modalità dimostrativa abilitata.</span>
            </div>
        <?php else: ?>
            <div class="banner banner-warning">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm.93-9.412-1 4.705c-.07.34.029.533.304.533.194 0 .487-.07.686-.246l-.088.416c-.287.346-.92.598-1.465.598-.703 0-1.002-.422-.808-1.319l.738-3.468c.064-.293.006-.399-.287-.47l-.451-.081.082-.381 2.29-.287zM8 5.5a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"/></svg>
                <span>Nessuna credenziale salvata in config.php. Visualizzazione dei dati dimostrativi (Demo Mode).</span>
            </div>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <div class="container">
        ...
        <header>
            <div class="brand">
                ...
                <p>Amministrazione WireGuard per MikroTik RouterOS 7</p>
            </div>
            
            <div class="status-badge">
                <span class="status-dot <?php echo $isDemoMode ? 'demo' : 'active'; ?>"></span>
                <span>Router CHR: <strong><?php echo $isDemoMode ? 'Simulato (Demo)' : htmlspecialchars($config['host']); ?></strong></span>
            </div>
        </header>
        ...
        <div class="stats-grid">
            <div class="stat-card" data-max-peers="<?php echo $maxPeers; ?>">
                <span class="stat-title">Total Peers</span>
                <span class="stat-value"><span id="stat-total-peers">-</span> <span id="stat-max-peers">/ <?php echo $maxPeers; ?></span></span>
                <span class="stat-desc">Configurati sull'interfaccia</span>
            </div>
            <div class="stat-card">
                <span class="stat-title">Subnet WireGuard</span>
                <span class="stat-value"><?php echo htmlspecialchars($config['subnet']); ?></span>
                <span class="stat-desc">IP Server: <?php echo htmlspecialchars($config['server_ip']); ?></span>
            </div>
            <div class="stat-card">
                <span class="stat-title">Endpoint Pubblico</span>
                ...
                <span class="stat-desc">Indirizzo per la connessione client</span>
            </div>
            <div class="stat-card">
                <span class="stat-title">Peer Attivi</span>
                <span class="stat-value" id="stat-active-peers">-</span>
                <span class="stat-desc">Handshake effettuato &lt; 5 min</span>
            </div>
        </div>
        ...
        <div class="control-panel">
            <div class="search-box">
                ...
                <input type="text" id="searchInput" placeholder="Cerca peer per nome o indirizzo IP...">
            </div>
            <button class="btn btn-secondary btn-sm" id="hideOfflineBtn" onclick="toggleHideOffline()" title="Nascondi peer offline" style="padding:0.5rem 0.85rem;font-size:0.85rem;">
                ...
                <span id="hideOfflineLabel">Nascondi offline</span>
            </button>
        ...
        <div class="table-card" style="position: relative;">
                ...
                <thead>
                    <tr>
                        <th id="th-name" onclick="sortPeers('name')" style="cursor:pointer;">
                            Nome <span id="sort-name-icon" style="font-size:.75rem;margin-left:4px;">↕</span>
                        </th>
                        <th id="th-ip" onclick="sortPeers('ip')" style="cursor:pointer;">
                            IP Assegnato <span id="sort-ip-icon" style="font-size:.75rem;margin-left:4px;">↕</span>
                        </th>
                        <th id="th-handshake" onclick="sortPeers('handshake')" style="cursor:pointer;">
                            Ultimo Handshake <span id="sort-handshake-icon" style="font-size:.75rem;margin-left:4px;">↕</span>
                        </th>
                        <th>Endpoint</th>
                        <th>Traffico</th>
            ...
            <div class="empty-state" id="emptyState" style="display: none;">
                ...
                <h3>Nessun peer trovato</h3>
                <p>Non è presente alcun client configurato su questa interfaccia WireGuard.</p>
            </div>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="addModalBackdrop">
        <div class="modal">
            <div class="modal-header">
                <h3>Aggiungi Nuovo Peer WireGuard</h3>
            ...
            <form id="addPeerForm" onsubmit="submitAddPeer(event)">
                <div class="modal-body">
                    <div id="modalFormContent">
                        <p style="color: var(--text-muted); font-size: 0.9rem; margin-bottom: 1.25rem;">
                            Verranno generate automaticamente le chiavi crittografiche client e assegnato il primo indirizzo IP libero disponibile all'interno della subnet.
                        </p>
                        <div class="form-group">
                            <label for="peerName">Nome Peer / Cliente</label>
                            <input type="text" id="peerName" placeholder="es. Enrico-Casa" required autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div id="modalResultContent" style="display: none;">
                        <div class="success-card">
                            ...
                            <div class="success-card-content">
                                <h4>Peer Creato con Successo</h4>
                                <p>Salva i dettagli o scarica il file di configurazione subito. Per ragioni di sicurezza, la chiave privata non sarà più visibile una volta chiusa questa finestra.</p>
                            </div>
                        </div>

                        <div class="form-group">
                            <label>Indirizzo IP Assegnato</label>
                            <div class="peer-ip-badge" id="resIp" style="font-size: 1rem; padding: 0.35rem 0.75rem;">-</div>
                        </div>

                        <div class="tab-buttons">
                            <button type="button" class="tab-btn active" onclick="switchAddTab('conf')">Configurazione (.conf)</button>
                            <button type="button" class="tab-btn" onclick="switchAddTab('script')">Script RouterOS (.rsc)</button>
                        </div>

                        <div id="tab-conf" class="tab-content active">
                            <div class="code-box">
                                <button type="button" class="copy-btn-code" onclick="copyToClipboard('code-conf-text')" title="Copia codice">
                        ...
                        <div id="tab-script" class="tab-content">
                            <div class="code-box">
                                <button type="button" class="copy-btn-code" onclick="copyToClipboard('code-script-text')" title="Copia codice">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75A1.125 1.125 0 0 1 4.875 9.75H8.25m2.25 2.25h9.75c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75c0-.621.504-1.125 1.125-1.125Z" /></svg>
                                </button>
                ...
                <div class="modal-footer" id="modalFooterActions">
                    ...
                    <button type="submit" class="btn btn-primary" id="btnSubmitAdd">Crea Peer</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="editModalBackdrop">
        <div class="modal">
            ...
            <form id="editPeerForm" onsubmit="submitEditPeer(event)">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="editPeerName">Nome Peer / Commento</label>
    ...
    <div class="modal-backdrop" id="deleteModalBackdrop">
        <div class="modal" style="max-width: 450px;">
            <div class="modal-header">
                <h3 style="color: var(--danger);">Elimina Peer WireGuard</h3>
                <button class="close-btn" onclick="closeDeleteModal()">&times;</button>
            </div>
            
            <div class="modal-body">
                <p>Sei sicuro di voler eliminare il peer <strong id="deletePeerNameText" style="color: var(--danger);"></strong>?</p>
                <p style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.5rem;">
                    Questa azione è irreversibile e rimuoverà la configurazione di accesso al server per il client.
                </p>
            </div>
            
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeDeleteModal()">Annulla</button>
                <button type="button" class="btn btn-danger" id="btnConfirmDelete">Elimina Ora</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="exportModalBackdrop">
        <div class="modal">
            <div class="modal-header">
                <h3>Esporta Configurazione Client</h3>
            ...
            <div class="modal-body">
                ...
                <div style="text-align:center; margin-bottom:1.25rem;">
                    ...
                    <p style="color:var(--text-muted); font-size:0.8rem; margin-top:0.5rem;">
                        Genera una nuova chiave privata, aggiorna il peer sul CHR e scarica la configurazione completa.
                    </p>
                </div>

                <div id="exportConfigSection" style="display:none;">
                    <div class="tab-buttons">
                        <button type="button" class="tab-btn" id="tabBtnExportConf" onclick="switchExportTab('conf')">Configurazione (.conf)</button>
                        <button type="button" class="tab-btn active" id="tabBtnExportScript" onclick="switchExportTab('script')">Script RouterOS (.rsc)</button>
                    </div>

                    <div id="tab-export-conf" class="tab-content">
                        <div class="code-box">
                            <button type="button" class="copy-btn-code" onclick="copyToClipboard('code-export-conf-text')" title="Copia codice">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75A1.125 1.125 0 0 1 4.875 9.75H8.25m2.25 2.25h9.75c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75c0-.621.504-1.125 1.125-1.125Z" /></svg>
                            </button>
                    ...
                    <div id="tab-export-script" class="tab-content active">
                        <div class="code-box">
                            <button type="button" class="copy-btn-code" onclick="copyToClipboard('code-export-script-text')" title="Copia codice">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75A1.125 1.125 0 0 1 4.875 9.75H8.25m2.25 2.25h9.75c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125v-9.75c0-.621.504-1.125 1.125-1.125Z" /></svg>
                            </button>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeExportModal()">Chiudi</button>
            </div>
        </div>
    </div>
    ...
    <div class="toast" id="toast">
        ...
        <span id="toastText">Operazione completata con successo!</span>
    </div>
